...
- **Security headers are split: static via `next.config.ts`, CSP via middleware**
  ...
  **Content-Security-Policy is strict and nonce-based**, set per-request in
  ...
  Never add `'unsafe-inline'` to `script-src`. Postgres connects over TLS when the
  connection string sets `sslmode` (`pgSslMode` in `packages/db/src/client.ts`;
  CA via `PGSSLROOTCERT`). At-rest encryption (Tiers 2–4) is deferred to Task 1.0.01.
...
- ✅ Task 0.5.18 — Registry contribution process: new `registry/plugins.json` (public discovery index — distinct from the internal `runtime/generated/registry.ts`; `{ registryVersion, plugins[] }` where each entry is a full plugin manifest). **The registry lists only third-party `sovereign`/`community` plugins; the built-in platform plugins (Console/Launcher/Account) are never registered — they ship in-repo and are always present. The array starts empty and grows by submission.** `registry/CONTRIBUTING.md` (submission requirements: valid manifest, public repo, LICENSE, compatible `minPlatformVersion`, unique id); a directory-based `.github/PULL_REQUEST_TEMPLATE/registry-submission.md` (the existing single `PULL_REQUEST_TEMPLATE.md` stays the default; the registry one is `?template=registry-submission.md`); `registry/__tests__/registry.test.ts` validates every entry via `validateManifest` and asserts each is `sovereign`/`community` (wired into `vitest.config.ts`, fails CI on an invalid or platform-typed entry); `docs/plugin-development.md` "Submitting to the registry" section. No version bump (docs/scaffolding) (merged to `main`).
- ⏳ Next: Task 0.5.19 — Stable SDK and semver commitment. (Tasks 0.5.10/0.5.12/0.5.13/0.5.14/0.5.17 are `[future]`.) Branch from an up-to-date `main`.
