...
- Task 1.11 — Test-user flag on seeded accounts (`@sovereignfs/db` → patch, `runtime` → patch, `plugins/console` → patch). `isTestUser` boolean added to `user.additionalFields` in `apps/auth`; Drizzle migration `0008_user_is_test_user` (SQLite + Postgres); seed script sets `is_test_user = true` for both seeded accounts; `GET /api/admin/users` includes `isTestUser`; Console → Users shows "Test" badge on seeded rows.
