...
- **`apps/auth` resolves its own dialect independently of `DB_DIALECT`** — purely from `AUTH_DATABASE_URL`'s scheme, since it deliberately doesn't depend on `packages/db`. `getAuthDb()`/`provisionAuthSqldNamespace()` call `assertAuthDialectMatchesPlatform()` before touching anything and throw on a mismatch — found as a real production gap where a Postgres-migrated platform silently left auth on its SQLite default because `AUTH_DATABASE_URL` was never set. A Postgres deployment must set `AUTH_DATABASE_URL` explicitly alongside `DB_DIALECT`/`DATABASE_URL`. Full detail: `docs/architecture-rules.md`.
