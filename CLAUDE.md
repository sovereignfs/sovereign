...
- **Version bumps** are part of the PR — bump the relevant `package.json`(s)
  ...
  to `1.0.0` marks the public release.** The current version is **`0.40.0`**
  ...
  via access token, RFC 0068's export completeness hardening, and required
  ...
  RFC's "Current state" section), and patch versions cover UI additions and
...
Current platform version: **`0.40.0`**. All roadmap tasks through slot `0.13.0` are complete; later minor bumps track post-slot tasks and patch versions are hotfixes.
