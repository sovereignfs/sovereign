...
migrate-to-postgres` (task 8.25) to fix that surfaced and fixed, in order: a
  ...
  dual-write-across-services pattern), RFC 0062's plugin mailer permission
  enforcement and `sdk.email.sendToUser()` surface, and opt-in single-key
  SQLite at-rest encryption (epic task 8.14, RFC 0071 —
  ...
  and
  most recently workstream 0008 leg 2b (research 0012, epic task 2.32) —
  cold-start offline launch: a `handlerDidError` plugin on the `pages` cache
  entry (`runtime/next.config.ts`) checks the offline session assertion from
  leg 2a and routes to a new `/offline/session-required` page when it's
  missing or expired, or to the existing generic `/offline` fallback when a
  valid session exists but nothing is cached yet — verified against the
  actual generated `sw.js`, not just type-checked. Extracted `useIsOffline`
  from `OfflineBanner` into `@sovereignfs/ui` (both now share one
  connectivity source) and added `OfflineGate`, which Console and Account
  wrap their content in to block administrative surfaces from rendering
  stale cached data while offline rather than trying to signal staleness.
  The login form swaps for a notice via the same hook when `/login` itself
  is viewed offline. Still outstanding: the real-device verification pass
  `docs/pwa-real-device-testing.md` calls for — this environment has no
  physical device access; before that workstream 0008 leg 2a (research 0012,
  epic tasks 1.21 + 2.31) — the offline session assertion and per-user
  service-worker cache partitioning that make cold-start offline safe to
  build on: the auth server signs a short-lived, asymmetric assertion
  (reusing the `jwt()` plugin's keypair, no new key material) that the
  service worker verifies with WebCrypto to key the `pages` cache per user,
...
--profile tools run --rm tools pnpm sv <command>` paid for a full Next.js
  ...
  `polling` to `sse`; before that the 2026-07-19 fix for
