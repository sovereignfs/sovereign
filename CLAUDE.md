...
- ✅ Task 0.5.18 — Registry contribution process: new `registry/plugins.json` (public discovery index — distinct from the internal `runtime/generated/registry.ts`; `{ registryVersion, plugins[] }` where each entry is a full plugin manifest; Console seeds it as the platform entry); `registry/CONTRIBUTING.md` (submission requirements: valid manifest, public repo, LICENSE, compatible `minPlatformVersion`, unique id); a directory-based `.github/PULL_REQUEST_TEMPLATE/registry-submission.md` (the existing single `PULL_REQUEST_TEMPLATE.md` stays the default; the registry one is `?template=registry-submission.md`); `registry/__tests__/registry.test.ts` validates every entry via `validateManifest` (wired into `vitest.config.ts`, fails CI on an invalid entry); `docs/plugin-development.md` "Submitting to the registry" section. No version bump (docs/scaffolding) (merged to `main`).
