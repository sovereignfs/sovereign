...
⏳ **Next: Task 0.5.28 — Accessibility audit & a11y contract (RFC 0025).** Add
`eslint-plugin-jsx-a11y` (recommended ruleset) to `eslint.config.ts`; four new
semantic tokens (`--sv-color-error/text`, `--sv-color-success/text`) + `prefers-reduced-motion`

- `--sv-color-focus-ring` in `packages/ui`; WCAG 2.1 AA audit & fix across
  runtime shell, `apps/auth`, Console, Launcher, Account; a11y sections in
  `docs/design-system.md` and `docs/plugin-development.md`; SRS NFR-11 entry.
  Branch from up-to-date `main` once PR #73 merges.
