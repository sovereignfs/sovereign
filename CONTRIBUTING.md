...
**Requirements:** Node.js 24.x, pnpm 11.5.2, Git, and **Docker** (must be
running before `pnpm dev` — see below; not just for the containerised-stack
QA path).
...
**Why Docker is required now:** the default `DB_DIALECT=sqlite` has no
plain-file fallback — every SQLite database (platform, auth, every plugin)
is served by a separate **sqld** container, the same way Postgres deployments
need a Postgres server. `pnpm dev` starts one for you automatically
(`scripts/ensure-sqld.ts`, a persistent `sovereign-sqld-dev` container reused
across runs — Docker just needs to be running, nothing to configure) before
handing off to `turbo dev`. If Docker isn't running, `pnpm dev` fails fast
...
or a manual clone into `plugins/<name>.local`), `--frozen-lockfile`
...
`pnpm test` runs the Vitest suite. In a default local checkout this needs no
external services — dialect-specific suites (`*.pg.test.ts`, `*.sqld.test.ts`)
are skipped unless their `TEST_DATABASE_URL`/`TEST_SQLD_URL` env vars are set;
in CI the same command receives both, so those suites run instead of being
skipped. Run it before a PR.
...
| sqld (SQLite) tests                | Package-local `*.sqld.test.ts` files              | `TEST_SQLD_URL=... TEST_SQLD_ADMIN_URL=... pnpm test` | Skipped unless both point at a disposable sqld instance. SQLite is always sqld-backed now — no plain-file fallback — so these are the SQLite dialect's real coverage.                                    |
...
```bash
pnpm test              # run Vitest tests; needs no external services by default
...
Similarly, SQLite has no plain-file fallback anymore — every SQLite-dialect
database is sqld-backed — so its real test coverage lives in **sqld tests**
(files named `*.sqld.test.ts`), skipped unless `TEST_SQLD_URL`/
`TEST_SQLD_ADMIN_URL` point at a live sqld instance:

```bash
docker run -d --name sov-test-sqld -p 28080:8080 -p 28081:8081 \
  -e SQLD_NODE=primary -e SQLD_ADMIN_LISTEN_ADDR='0.0.0.0:8081' \
  ghcr.io/tursodatabase/libsql-server:latest /bin/sqld --enable-namespaces
TEST_SQLD_URL=http://localhost:28080 \
  TEST_SQLD_ADMIN_URL=http://localhost:28081 \
  pnpm test
docker rm -f sov-test-sqld
```

Use a single-label hostname (`localhost`, never a raw IP like `127.0.0.1`) —
with `--enable-namespaces`, sqld infers the default namespace from the Host
header's first dot-separated label, so an IP address gets misparsed as a
namespace name and every unnamespaced query 404s. CI runs these too, the same
way as the Postgres parity tests — see `.github/workflows/ci.yml`'s `test` job.
...
```bash
git clone git@github.com:me/sovereign-tasks.git plugins/sovereign-tasks.local
pnpm install   # links the newly cloned workspace deps
pnpm dev       # composes them into the runtime
```

This directory is yours alone: it is never committed and no code references
it, so the specific plugins you clone stay private to your machine. Managing a
personal list of repos to clone this way across checkouts is handled by the
`sovereignfs/sovereignfs` workbench's `workbench plugins pull` command, not by
this repo.
