...
beforeEach(() => {
  process.env.RELAY_ENROLLMENT_SECRET = 'test-enrollment-secret';
});
...
describe('issueEnrollmentToken / verifyEnrollmentToken', () => {
  ...
  it('rejects a token signed with a different secret', async () => {
    ...
    process.env.RELAY_ENROLLMENT_SECRET = 'a-different-secret';
  ...
  it('rejects malformed input without throwing', async () => {
    ...
    expect(verifyEnrollmentToken('not-a-jwt')).toBeNull();
  ...
  it('rejects a signature of the wrong length rather than throwing', async () => {
    ...
    expect(verifyEnrollmentToken(`${header}.${payload}.short`)).toBeNull();
  });
});
