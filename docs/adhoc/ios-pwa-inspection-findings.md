...
> **On-device verification (2026-07-16).** A developer reported the flash still visible on a real iPhone 15 Pro, installed to Home Screen (standalone launch, not a Safari tab). Investigation confirmed the splash assets and media queries were not the problem — the iPhone 15 Pro's exact profile (`393×852`, dpr 3) has matching light/dark/portrait/landscape PNGs on disk and correct `<link>` entries in `apple-splash.ts`. Two real contributors were found instead:
>
> 1. `body`'s painted background (`globals.css`) used `--sv-color-surface-sunken` (`#fafafa`/`#18181b`) while the splash images, manifest `background_color`, and the pre-paint `theme-color` meta all use `--sv-color-surface` (`#ffffff`/`#09090b`) — a visible color shift in the pre-`.shell`-mount window. Fixed by aligning `body`'s background to `--sv-color-surface`; safe because `.shell` fills `100dvh` with its own backgrounds once mounted, so `body`'s own color is only ever visible during that transient gap.
> 2. The service worker's default `pages` cache entry is `NetworkFirst` with no `networkTimeoutSeconds`, so every standalone launch does a live network round-trip for the SSR document before anything can paint — on a bad connection this could hang indefinitely rather than falling back. Bounded with `networkTimeoutSeconds: 4` in `runtime/next.config.ts` (falls back to the cached `/offline` shell). This does not eliminate the fast-network case: iOS dismisses the native launch image on its own readiness heuristic, ahead of the page's actual paint, and there is no web API to control the WKWebView's initial backing color — a residual sub-second flash is a known, largely irreducible WebKit limitation for non-native-wrapped PWAs (see the planned Capacitor shell, CLAUDE.md "Native mobile app"). Deliberately did **not** switch `pages` to a stale-serving strategy (`StaleWhileRevalidate`) — see `docs/architecture-rules.md`'s PWA rule for why (per-user SSR content, multi-user risk).
