...
| `Dialog`              | `role="dialog"`                              | Esc close, Tab/Shift-Tab trap                                                         | `aria-modal="true"`; accessible name via `DialogTitle` (`aria-labelledby`, takes priority), else `aria-label`, else `title` | First focusable on open; restore on close                             |
...
1. **Location:** `src/components/<Name>/<Name>.tsx` with a co-located
   `<Name>.module.css` and a `<Name>.test.tsx` under `src/components/<Name>/__tests__/`
   — matching the per-directory `__tests__/` convention documented in the root
   `CLAUDE.md`, not a file directly co-located next to the component.
...
| ----------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| `Dialog` (sm/md/lg/full/auto) | Centered modal — fixed size, or `auto` sized to content     | Full-page modal page between shell header/footer, slide transition, `OverlayHeader` on top       | Modal: the user must act before continuing (overlay-shell plugins — Console, Account) |
...
Every component ships its own `<Name>.stories.tsx`, co-located in its
component directory, covering at minimum the default state and one
meaningful variant (the Storybook hygiene rule in the root `CLAUDE.md`). A
handful of components whose stories predate that per-directory convention
instead live under `src/stories/` (e.g. `Avatar`, `Card`, `Checkbox`,
`FormField`, `PageContainer`, `Textarea`) — both locations are equally
canonical; check the component's own directory first, then `src/stories/`.

This section deliberately does **not** duplicate a per-component table of
story files and their variants, the way it used to: that table required a
manual update on every new component or added variant and had drifted badly
by the time it was last audited (it listed roughly 34 of the 83 components
actually shipping a story, and nothing enforced it staying in sync). The
**Component Gallery** section of `DesignSystemOverview.stories.tsx` is the
actual canonical, enforced index — every shipped component has an entry
there, per the per-PR Storybook hygiene rule — so it's the one place this doc
points to instead of a second copy that can silently rot.
