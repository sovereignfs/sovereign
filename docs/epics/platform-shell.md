...
- Convert the `notifications` table definition to the 3-arg `pgTable`/`sqliteTable` form and add two indexes in both `packages/db/src/schema/postgres/platform.ts:218-232` and `packages/db/src/schema/sqlite/platform.ts:497-518`, matching the existing `plugin_jobs`/`backup_jobs` index pattern. **Correction from this task's original text** (found while implementing: the queries don't support the originally-proposed single `(tenant_id, recipient_user_id, dismissed_at)` index — `listUserNotifications` sorts `created_at DESC` and `countUnreadNotifications` filters `read_at IS NULL`, neither leads or sorts on `dismissed_at`) — port `packages/db/src/bootstrap.ts`'s two existing raw-SQL indexes verbatim instead: `index('notifications_user_feed').on(table.tenantId, table.recipientUserId, table.createdAt)` (Postgres adds `.desc()` on `createdAt`; SQLite's `index()` builder has no `.desc()` column modifier in the installed `drizzle-orm` version, but a plain ascending index is scanned backward for free by SQLite's planner) and `index('notifications_unread').on(table.tenantId, table.recipientUserId, table.readAt)` — this covers `listUserNotifications` and `countUnreadNotifications` (`packages/db/src/platform-db.ts:2762-2791`) exactly, and keeps a bootstrap-created DB (test-only, `:memory:`) and a migration-created DB converging on the same index names instead of diverging.
