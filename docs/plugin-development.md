...
| `shellConfig`          | object (see below)                                      | no                                   | Per-shell tuning. Holds `overlaySize` (`sm` \| `md` \| `lg` \| `auto`, default `lg`) for `shell: overlay` plugins, and `mobileHeader`/`mobileFooter` (booleans, default `true`) for `shell: default` plugins (RFC 0075). Each field is only valid for its own `shell` value.                                                                                                                                       |
...
- The dialog size is set by `shellConfig.overlaySize` (`sm` | `md` | `lg` |
  `auto`, default `lg`), matching the `Dialog` component's own full size
  set. `sm`/`md` are fixed width with content-driven height (each capped);
  `lg` is a true fixed box (100%/100%) — the panel never resizes as you
  navigate between sub-routes, at the cost of always filling the viewport;
  `auto` is content-driven on both width and height (each capped) — use it
  for a panel that shouldn't fill the viewport but also isn't a good fit
  for a fixed box sized around one specific sub-route's content.
