...
| `disabled`             | boolean                                                 | no (default `false`)                 | Hard-disables the plugin unconditionally — a build-time author declaration, distinct from the runtime/Console-controlled per-instance enable toggle, which no admin action can override and which is never bypassed in local dev either. Routes stay built but are unreachable, migrations are skipped, and Console cannot activate it. Use to take a shipped plugin fully out of reach without deleting its code. |
...
- The dialog size is set by `shellConfig.overlaySize` (`sm` | `md` | `lg` |
  `auto` | `fixed`, default `lg`), matching the `Dialog` component's own full
  size set. `sm`/`md` are fixed width with content-driven height (each
  capped); `lg` is a true fixed box (100%/100%) — the panel never resizes as
  you navigate between sub-routes, at the cost of always filling the
  viewport; `auto` is content-driven on both width and height (each capped)
  — a plugin whose route tree includes a client-side redirect through a
  near-empty intermediate page will see this size visibly shrink to fit
  that page, then grow once real content lands; `fixed` is also a true
  fixed box like `lg` (same "never resizes" guarantee) but capped rather
  than filling the viewport — for a panel that shouldn't fill the viewport
  but also isn't a good fit for a content-driven size (Account's own case,
  epic task 14.5).
