...
**Status:** v0.1 implemented — Profile, Security, and Preferences (ACC-01–11; see Changelog for per-requirement task attribution)
...
change, active sessions, TOTP/passkey security), and preferences (timezone,
appearance, sidebar customisation).
...
| ACC-10 | Sidebar customisation: pin, unpin, and reorder plugins in the sidebar middle section. Preferences stored in `account_prefs.sidebar_plugins` (task 2.13).                                                              |
...
| 0.1     | Sep 2026 | Corrected the Functional requirements section: ACC-09 (TOTP, task 1.4/RFC 0012, shipped at platform v0.5.27) and ACC-10 (sidebar customisation, task 2.13, shipped at platform v0.9.3) were still listed under a "v0.2 — Security and customisation (post-v1)" heading as unbuilt, despite both being live in the shipped code (`TotpSection.tsx`/`PasskeySection.tsx` in `security/page.tsx`; `SidebarControl.tsx` in `preferences/page.tsx`) — confirmed by reading both route files directly, not assumed from file presence alone. Moved both into the v0.1 Core tables (Credentials and Preferences respectively) and removed the now-empty v0.2 heading; corrected ACC-10's own wording, which had described a since-abandoned separate `account_sidebar_prefs` table rather than the `account_prefs.sidebar_plugins` column task 2.13 actually shipped. |
