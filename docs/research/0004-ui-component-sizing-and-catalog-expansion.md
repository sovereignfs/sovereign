# Research 0004 — Expanding `packages/ui`'s sizing scale and component catalog
...
Should Sovereign's design system be reworked toward a denser, more
desktop-web-conventional sizing scale and a broader component catalog —
sizing/scale, a compound composition API, and 70+ components in total —
while staying mobile-first, PWA-ready, and free of third-party
component-framework dependencies (per CLAUDE.md)?

Decision inputs already given by the developer:

1. **Fidelity target:** sizing/visuals only, not full interaction-behavior
   parity with any external reference.
2. **Existing component APIs:** rework toward a compound pattern
   (`TabsList`/`TabsTrigger`/`TabsContent`-style), accepting this is a breaking
   change.
3. **Catalog scope:** a broad catalog of 70+ components, not just the curated
   generic subset.
...
`packages/ui/src/components/` has 43 components. Direct equivalents already
exist for the common generic set: Avatar, Badge, Button, Calendar, Card,
Checkbox, Date Picker, Dialog, Drawer, Dropdown Menu → `Menu`, Empty →
`EmptyState`, Field → `FormField`, Input, Popover, Select, Sheet, Spinner,
Tabs, Textarea, Toggle, Tooltip, Alert Dialog → `ConfirmDialog`. The
remainder of our inventory is product-specific (`CurrencyInput`, `SplitPane`,
`TagInput`, `IconPicker`, `MemberMultiSelect`, `SegmentedControl`,
`SystemBanner`, `BalanceChip`, `StatusBadge`, shell chrome like
`OverlayHeader`/`PageHeader`/`NavTabs`) with no generic-catalog counterpart,
and stay out of scope for this rework.
...
- **No third-party component framework** (CLAUDE.md). A denser catalog in
  this space is typically a thin CSS layer over a headless interaction
  library that supplies keyboard nav, roving tabindex, focus trapping,
  portal + collision-aware positioning, and typeahead. We cannot import that
  kind of headless library; any behavior it would supply "for free," we'd
  have to hand-build in vanilla React + CSS Modules.
...
- **DS-first mobile interaction primitives already exist** — long-press,
  double-tap, `useIsMobile` (CLAUDE.md's "DS-first" rule) — built because the
  desktop-oriented catalogs this research surveyed have no equivalent; their
  components assume a mouse cursor.

### The fidelity/scope combination has an internal tension

"Sizing/visuals only" and "a broad catalog" pull against each other for one
sub-group of components. For simple, mostly-static components (Switch, Radio
Group, Slider, Progress, Separator, Skeleton, Table, Alert, Breadcrumb,
...
ship them without building full custom behavior in-house (since a headless
interaction library is off the table), which is a much larger undertaking per
component than a visual port — closer to designing and building a new
interactive primitive from scratch than "matching a reference catalog."
Several of them also don't have a mobile/touch equivalent to match in the
first place — see below.

### Mobile/PWA-readiness conflicts, not just gaps

- **Touch target size.** The default control heights common in this class of
  desktop-oriented component catalog (roughly 32–36px) are tuned for a mouse
  cursor and sit below the 44px (iOS HIG) / 48dp (Material) minimum
  recommended touch target. Literally matching those pixel values would read
  fine on desktop but be borderline-small on mobile — this directly conflicts
  with the "mobile ready" requirement and needs an explicit decision (see
  Open questions).
- **No touch equivalent for cursor-only affordances.** Hover Card
  ...
  analog — "match the reference catalog" doesn't apply because the input
  model they're built for doesn't exist on mobile. Navigation Menu/Menubar's
  hover-triggered flyouts have the same problem. These need original mobile
  interaction design (e.g. long-press for context menu, tap-to-expand for
  hover card), not a port.
...
1. **Sizing/scale alignment on existing components** — bring the sizing
   scale on current flat components onto a denser, more conventional rhythm
   via token changes. Additive/visual only, no API change, no version-bump-
   worthy break by itself (though visual changes to a public DS are still
   worth a minor bump + a design-system.md/changelog note for consumers, per
   NFR-04's spirit even if not its letter).
...
4. **Behavior-heavy / desktop-shaped components** — Command, Context Menu,
   ...
   catalog port) before it's buildable — closer to a mini-RFC per component
   than a rework task.
...
"sizes should be denser/more conventional" and "a broad catalog" (numerically,
phases 1–2 already cover ~20 of the ~30 gap) without touching the
breaking-change or novel-interaction-design risk at all. Not chosen as _the_
recommendation because the developer has already indicated the compound-API
rework and full catalog are wanted — but flagged as the lower-risk fallback
if phases 3–4 prove too costly once scoped.

## Recommendation

Adopt **Option B** (phased). Sequence phases 1 and 2 first — they deliver the
literal "sizes should be denser" ask immediately, are additive, and de-risk
nothing else. Treat phase 3 (compound-API rework) as a separate
...
per component** before any sizing work is meaningful — "match the reference
catalog" doesn't resolve the core question for these, which is "what does
this even do on touch," so scope them last and expect their timeline per
component to look more like a new feature than a component port.
...
Font-size and radius primitives already track a common 4px/8px spacing rhythm
closely (14px body text, 8px `md` radius, etc.) with no touch-target
implication at all.

**Decision:** match the denser reference scale literally where there's no
touch-target concern (font-size, radius, spacing rhythm). For every
interactive control, keep the visual size matched to that scale but require
the same `min-height`/hit-area pattern Button already has. No literal-vs-mobile
...
| full Data Table          | Out      | Phase 2's plain `Table` covers the real need; sort/virtualization is a data-grid feature, build only against a real consumer    |
