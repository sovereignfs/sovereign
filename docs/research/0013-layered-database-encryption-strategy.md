...
Graduated: **[RFC 0092](../rfcs/0092-app-level-field-encryption.md)** (Draft) carries the design — the `sdk.crypto.encryptField()`/`decryptField()` surface, the `encryptedText()`/`blindIndex()` schema helpers, the KEK→DEK envelope hierarchy, and the sensitivity-taxonomy + policy-flag model — with the backfill migration designed as a bounded, operator-triggered step from day one per the RFC 0071 precedent. Implementation is sequenced as **[workstream 0011](../workstreams/0011-app-level-field-encryption.md)** (epic tasks 8.31–8.34); this doc's open questions became RFC 0092's open questions and the workstream's gates A/B. Layer 3 (RFC 0060) needs no further RFC work — only continued documentation of when a plugin should reach for it over Layer 2.
