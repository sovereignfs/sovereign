...
> **Correction (2026-09-06):** as shipped (epic task 1.9), `minVerificationLevel` is an
> unconditional hard gate at the route boundary for every level — a 303 to
> `/verification-required/<pluginId>` for pages, a 403 for API routes — with no soft-banner
> mode. `AUTH_REQUIRE_MFA` therefore never had a role to play and was retired without ever
> being consulted; see changelog 0.4. An instance-wide "every user must enrol MFA" policy is
> a different feature (a forced-enrollment flow at sign-in), not this flag.
...
| 0.4     | September 2026 | `AUTH_REQUIRE_MFA` retired (root 0.130.6). It was read into `apps/auth`'s config in Phase 1 and consulted nowhere — Phase 2 never named an enforcement point, and the `minVerificationLevel` gate it was meant to switch from soft to hard shipped as always-hard, so there was nothing left for it to switch. Removed from `apps/auth/src/env.ts`, `.env.example`, and `docs/self-hosting.md` rather than left as a documented no-op. Forced instance-wide MFA enrolment, if ever wanted, is a separate feature.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
