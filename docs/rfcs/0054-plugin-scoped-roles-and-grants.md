...
| 0.2     | August 2026 | Task 1.13 implemented locally (workstream 0017 leg 3), not yet committed. Resolved the open questions above using this RFC's own §4 "Proposed design" recommendation and existing codebase precedent, not new invention: open question 1 (reusable DB tables vs. SDK-only) → SDK-only, plugin-owned storage, mirroring `sdk.portability.provideExport/Import/Delete`'s exact registration pattern; open question 2 (`roles` in manifest now vs. docs-first) → added now, per this RFC's own adoption-path step 2 and the epic task's deliverable list; open question 3 (dotted vs. kebab-case capability names) → kebab-case, matching every existing capability name in the codebase (`^[a-z][a-z0-9-]*$`), not this RFC's dotted `project.edit` example; open question 5 (emergency override in v1) → not built, per §7's own "does not require an owner override in v1"; open question 6 (Account "resources shared with me") → not built, per the adoption path's own "consider only after at least one real plugin implements the pattern." Open question 4 (import user-matching) resolved as documented guidance in `docs/plugin-development.md`, not new platform code — matches §9's existing safe-match-or-inert-metadata rule via the existing portability hooks (Task 8.8). |
