...
- **Real memory beyond recency truncation.** Context-window handling is
  ...
  scope here.
- ~~**Provider "test connection" UX.**~~ **Resolved (task 22.13).** Saving
  runs a live `/models` call first: a rejected key (401/403) is refused with
  the form intact — that is a mistake only fixable right there — while an
  unreachable host is saved anyway and reported as a warning, since a
  self-hosted server may simply be off. Success reports the model count.
- ~~**Model list cache TTL / manual refresh.**~~ **Resolved (tasks 22.12,
  22.13).** A 30s per-user cache, deduped across concurrent callers and
  served stale-while-revalidate up to 5 minutes; the explicit "Recheck"
  actions invalidate it and run live.
...
- **RFC 0040's full revisit remains open.** This RFC resolves RFC 0040's
  ...
  it.
- ~~**Sidebar cutoff beyond 10 sessions.**~~ **Resolved (task 22.13).** The
  Recent group shows ten and folds the rest behind "Show more"; once there
  are more chats than fit, a search box over every chat (pinned or not)
  appears. Nothing is unreachable from the sidebar any more.
...
| 0.9     | September 2026 | Launch performance, streaming correctness and UX pass (epic task 22.13): the chat shell streams instead of blocking on model discovery; discovery is deduped and served stale-while-revalidate; the first reply of a new chat is no longer lost to a mid-stream remount, and Stop cancels the provider request; reply controls (Continue for a cut-off reply, Regenerate, Edit) and per-message model/time; the sidebar becomes a Sheet on mobile. Three open questions resolved at implementation time: provider connection check on save, discovery cache policy, and the sidebar cutoff (Show more + search)                                                                                                                                                                                   |
