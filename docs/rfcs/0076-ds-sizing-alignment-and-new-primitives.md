...
[Research 0004](../research/0004-shadcn-spec-component-expansion.md).\
...
Align `packages/ui`'s sizing scale with shadcn/ui's spec (control heights,
spacing rhythm, font sizes, radii) across existing components, and add 13 new
primitive components that exist in shadcn's catalog but not ours: Switch,
Radio Group, Slider, Progress, Separator, Skeleton, Table, Alert, Breadcrumb,
Pagination, Kbd, Accordion, Collapsible. Both parts are additive — no
existing component's public API changes, only visual sizing and net-new
exports.

## Motivation

Research 0004 found that porting shadcn's _spec_ wholesale conflates three
different-risk changes. This RFC scopes the lowest-risk one: shadcn is a
useful sizing/rhythm reference, and Sovereign is missing a set of boring,
...
- `packages/ui/src/tokens/primitives.css` already defines a scale that tracks
  Tailwind's (and by extension shadcn's) numbers closely: `--sv-font-size-sm`
  is 14px, `--sv-radius-md` is 8px, spacing steps follow the same 4px/8px
  rhythm. This RFC does not propose new primitive tokens — the scale is
  already close enough that alignment is mostly about _applying_ it
  consistently to component dimensions, not redefining it.
...
padding/radius, `Card` padding/radius) against shadcn's default/sm/lg scale
and adjust CSS Module values to match, using the Button pattern as the
template: visual size matches shadcn, `min-height`/hit-area stays at or above
`--sv-touch-target-min`. Where a component doesn't yet expose size variants
(sm/md/lg) that shadcn does, this RFC does not add them speculatively —
...
| `Table`       | Plain semantic table (`thead`/`tbody`/`tr`/`td` wrappers with DS styling) — explicitly not shadcn's Data Table (sort/virtualization); see Research 0004's "full Data Table — Out" decision                      |
...
**Skip the sizing audit, only add new components sized to match shadcn.**
Rejected — would leave existing and new components visually inconsistent
with each other (e.g. a shadcn-sized `Switch` next to an unaligned `Button`),
...
into epic tasks 25.1–25.6 in the new [shadcn Component Parity](../epics/shadcn-component-parity.md)
