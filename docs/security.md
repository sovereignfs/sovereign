...
| Stolen disk / leaked backup / VPS snapshot                                | **Opt-in SQLite at-rest encryption** (RFC 0071, `SOVEREIGN_DB_ENCRYPTION_KEY`) for `sovereign.db`/`auth.db` always, plus any isolated plugin DB whose manifest requests it (`database.requireEncryption`, per-database enforcement — task 8.15); host-level disk encryption for everything else (Postgres, avatars/blobs, non-requesting plugin DBs); full envelope/field-level at-rest post-v1 (Task 8.5) | **v1 (opt-in, SQLite)** / post-v1 (rest)           | Off by default — without it enabled, rely on host-level disk encryption. Even enabled, server-held keys never defend against a curious operator or RCE (see the client-side row below for that).                                                                             |
