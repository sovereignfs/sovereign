...
| `SOVEREIGN_ENCRYPT_CLASSES`                                    | no                       | —                                                   | App-level field encryption policy (RFC 0092) — comma-separated sensitivity classes to encrypt (e.g. `pii,health`). Unset/empty = field encryption off, the platform behaves exactly as before. When non-empty, `SOVEREIGN_FIELD_KEK` becomes required and startup fails loudly without it. Enabling a class affects **new writes only** — pre-existing rows are never rewritten automatically (the explicit operator-run backfill tool ships with workstream 0011 leg 4). Runtime service only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
