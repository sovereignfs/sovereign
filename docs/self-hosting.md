...
| `SOVEREIGN_BACKUP_WORKER_ENABLED`                              | no                       | —                                                   | Set to `1` (or `true`) to start the platform backup-job worker (RFC 0084, epic task 8.16). **Off by default**, unlike `SOVEREIGN_SCHEDULER_DISABLED`/`SOVEREIGN_JOB_WORKER_DISABLED` above (which are opt-_out_) — this feature has no enqueue path yet (Console/Account UI not shipped) and instance-scope jobs cannot succeed in the documented production Docker deployment yet, so leaving it running by default would just be a wasted DB query every 60s on every instance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
