...
| `SOVEREIGN_VAULT_KEY`             | no       | —                                                   | 32-byte key for the plugin secret vault (`sdk.secrets`). Required only when plugins create/read runtime secrets. No default; generate with `openssl rand -base64 32`. Losing or rotating it without re-encrypting vault rows makes existing vault values unreadable.                                                                                                                                                                                                |
