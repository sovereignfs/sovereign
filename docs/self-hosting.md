...
| `SOVEREIGN_OFFLINE_SESSION_TTL_SECONDS` | no       | `1209600` (14 days)                                 | How long an offline session assertion stays valid (research 0012). **This value is the offline revocation gap** — a device that goes offline keeps rendering its cached shell for this long after the server revoked the session, because no server code runs to tell it otherwise. Lower it to tighten that window at the cost of offline reach. Clamped to 1 hour…90 days; never outlives the session itself. Distinct from the 300s session cookie cache, which bounds role-change staleness for _online_ requests.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `SOVEREIGN_RELAY_URL`                   | no       | `https://relay.sovereign.openfs.io`                 | Deployment-time default for the Sovereign Relay (RFC 0087) that new `sovereign-mobile` device-token registrations use — unrelated to Web Push above, which needs no relay. An admin can override this from Console (takes precedence over this env var); only relevant if you're self-hosting your own `apps/relay` instance, which for push specifically also requires your own Apple/Firebase credentials and a separately signed app build, since APNs/FCM tokens are permanently bound to the app identity that generated them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
