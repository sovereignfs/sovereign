...
- First-party external support repositories for the default Tasks plugin, example
  plugins, plugin template, Storybook deployment, infra template, docs
  deployment, and archived legacy codebase. See
  [docs/repositories.md](repositories.md) for the current canonical map.
...
**Sovereign Legacy (v1):** Node.js / Express / Handlebars monolith, SQLite, Prisma. Technology-agnostic plugin aspiration. Reached meaningful maturity (481 commits, Docker, PM2, CLI, migrations) but the architecture ceiling was low and the stack made a capable plugin system difficult to build. Archived at [`sovereignfs/sovereign-legacy`](https://github.com/sovereignfs/sovereign-legacy).
...
| PWA                 | Serwist service worker (`runtime/worker/`, built by `runtime/scripts/build-sw.ts`)           | Service worker, installable shell                                                                                                                                                                                                                                                                        |
...
#### Sovereign reference plugins (historical planning)

The current first-party support repository map lives in
[docs/repositories.md](repositories.md). The older candidate plugin roadmap below
is retained as historical planning context; only repositories listed in the
repository map are current first-party support repositories.
...
**sovereign-plugin-splitify** (historical candidate)
...
**sovereign-plugin-plainwrite** (historical candidate)
...
**sovereign-plugin-apicomposer** (historical candidate)
...
- The **runtime** is exposed through the reverse proxy as the main workspace
  origin.
- The **auth server** has a browser-facing public origin for login, callbacks,
  and WebAuthn compatibility routes. Runtime server-to-server calls still use
  the internal Docker network at `http://auth:<port>` via
  `SOVEREIGN_AUTH_URL`.
...
- `"overlay"` — the plugin renders as a dismissable dialog **over** the current page rather than navigating away from it (a quick, interruption-style layer for settings-like or quick-capture plugins; Account is the motivating case — Console rendered this way too until workstream 0022 moved it to `shell: "default"`). URLs are real and unchanged; a hard/direct load renders the plugin as a full page (the `default`-shell fallback). Implemented via App Router parallel + intercepting routes — the plugin's `app/` tree composes twice (an interception copy for soft navigation, a full-page fallback for hard loads). The runtime owns the dialog chrome; plugins write ordinary pages and flip one manifest field. The dialog size is plugin-declared via `shellConfig.overlaySize` (`sm`/`md`/`lg`/`auto`/`fixed`, default `lg`): `sm`/`md` are fixed-width, content-driven-height boxes (each capped); `lg` and `fixed` are both true fixed boxes (width and height both set, content never resizes them) — `lg` fills the viewport minus a fixed margin, `fixed` is capped at 64rem×44rem instead, for a panel that shouldn't fill the viewport but also needs a constant footprint (Account's own case — its `/account` → `/account/profile` client-side redirect through a near-empty intermediate page ruled out a content-driven size, which would visibly shrink to fit that page and then grow); `auto` sizes to content on both width and height, each capped, for a panel whose footprint genuinely varies a lot with no such redirect gap in its own route tree. Dismissal is a single `router.back()`, so a plugin's intra-overlay navigation (tabs/sections) must use `replace` to keep the overlay on one history entry. See RFC 0001.
...
The runtime ships a Serwist-based service worker (`runtime/worker/`, bundled by `runtime/scripts/build-sw.ts` after `next build`; originally `@ducanh2912/next-pwa`, replaced on the Next 16 upgrade because it is webpack-only). The PWA shell is installable from the browser. The service worker caches the shell and static assets for offline availability. Plugin data is cached offline only for plugins that opt in: a plugin declaring `offline: true` in its manifest has its bare `routePrefix` precached and may persist data client-side through `sdk.offline` (read cache) and `sdk.offline-queue` (mutation queue), both IndexedDB-backed — see RFC 0074 and RFC 0078. A plugin that does not declare the field gets no offline data caching, which remains the default.
...
interface SovereignManifest {
  ...
  // content-driven-height boxes (each capped); "fixed" is also a true fixed box (width and
  // height both set, never resizes with content) but capped at 64rem×44rem rather than
  // filling the viewport; "auto" sizes to content on both width and height (each capped) —
  // for a panel whose footprint genuinely varies a lot, accepting that it will visibly
  // resize as content changes (including across a client-side redirect through a
  // near-empty intermediate route — "fixed" has no such gap).
  shellConfig?: {
    overlaySize?: 'sm' | 'md' | 'lg' | 'auto' | 'fixed';
  };
...
| Jun 2026 | npm-published packages: `@sovereignfs/sdk`, `@sovereignfs/ui`, and `@sovereignfs/create-plugin`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `sdk` is the plugin↔platform contract — external plugin developers must install it. `ui` is the design system — plugin developers install it to use components and tokens. `create-plugin` is the scaffolding CLI (`npm create @sovereignfs/plugin`) — a dev tool, not a runtime library. All other packages (db, manifest, mailer, tsconfig) are workspace-internal infrastructure; publishing them would expose platform internals with no benefit. They are marked `"private": true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Jun 2026 | Auth has a browser-facing origin; runtime uses internal auth URL for server calls                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Runtime is the main workspace origin. Auth is exposed through the reverse proxy for login, callbacks, compatibility routes, and WebAuthn origin handling. Runtime server-to-server calls still use `SOVEREIGN_AUTH_URL` (`http://auth:3001`) on the internal Docker network, and `AUTH_TRUSTED_ORIGINS` allows those internal-origin POSTs when `AUTH_BASE_URL` points at the public auth domain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Jun 2026 | Fixed internal ports (3000 runtime / 3001 auth); host ports differ by env (4000/4001 default in prod)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Containers always listen on 3000/3001 internally; the dev-vs-prod distinction is a host port mapping concern, not an app config change. Production host defaults are 4000 (runtime) and 4001 (auth), normally reached through a reverse proxy such as Caddy. Overridable via `RUNTIME_PORT` / `AUTH_PORT`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Jun 2026 | npm publishing: per-package version tags trigger a CI publish job                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | The three published packages have independent release cycles, so each is released on its own `<slug>-vX.Y.Z` tag (`sdk-v*`, `ui-v*`, `create-plugin-v*`) rather than a single repo-wide tag. Root platform releases use plain `vX.Y.Z`; package/app/plugin releases use the same `<slug>-vX.Y.Z` pattern (for example `docs-vX.Y.Z` for the docs app). A GitHub Actions job builds the tagged package with tsup and runs `pnpm publish` using a `NODE_AUTH_TOKEN` secret. No other packages are published.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
