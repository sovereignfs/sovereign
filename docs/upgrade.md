...
- **better-auth 1.6.25 → 1.6.30** (`better-auth`, `@better-auth/passkey`,
  `@better-auth/oauth-provider`). Same 1.6 line, no schema change, no action
  required.
- **better-auth 1.7.0–1.7.2 are deliberately skipped** (epic task 1.26). Those
  three releases add a required `account.issuer` column that better-auth's
  migrator refuses to add to any table with existing rows — the auth server
  would fail to boot on every real instance — and better-auth's own 1.7
  upgrade guide states 1.7.3 removes the column again. `dependabot.yml` now
  ignores that window. **If you upgraded an instance to 1.7.0–1.7.2 by hand**
  and it booted (a fresh install with no accounts at the time), follow
  better-auth's "Account identity keeps the provider key" guide before moving
  to 1.7.3 or back to 1.6: on SQLite, `DROP INDEX account_issuer_accountId_uidx;`
  then `ALTER TABLE account DROP COLUMN issuer;`.
- Two genuine `session_data` cookies (captured from real better-auth 1.6.25
  and 1.7.2 instances) are now permanent fixtures in the runtime's
  session-verification tests, so a future bump that changes the cookie-cache
  format the middleware verifies offline fails CI instead of logging every
  user out.
...
| 0.99.1          | better-auth 1.6.30; 1.7.0–1.7.2 skipped (required `account.issuer` column breaks boot on populated stores, reverted upstream in 1.7.3); genuine 1.6.25/1.7.2 cookie-cache fixtures added to `session-verify.test.ts` (epic task 1.26, parked) |
