...
| `0.9.4`      | Instance identity rename (epic task 9.8, RFC 0032)                                   |
| `0.9.x`      | Remaining pre-v1 tasks — see [`docs/roadmap.md`](roadmap.md) for current ordering    |
...
**For remaining pre-v1 tasks and their current priority order, see [`docs/roadmap.md`](roadmap.md).** That file is the single source of truth for what ships and when.
...
1. **Complete remaining pre-v1 tasks** — see [`docs/roadmap.md`](roadmap.md) for
   the current list and order. Each task bumps root `package.json` by one patch.
...
5. **Update `docs/upgrade.md`** with final transition notes to `1.0.0`.
