...
| 0.5     | August 2026 | Leg 4 shipped (task 3.23). Split `scripts/generate-registry.ts` into nine modules under `scripts/generate/` — the epic's six (`read-plugins`, `compose-routes`, `plugin-icons`, `plugin-env`, `plugin-capabilities`, `write-registry`) plus `paths.ts`/`types.ts` and three more discovered during implementation (`plugin-schedules.ts`, `plugin-jobs.ts`, `plugin-events.ts` — the source file had three more generated-output concerns than the deliverable list named). `scripts/generate-registry.ts` stays the CLI entrypoint and re-exports every module's public API, so Task 3.22's existing 55-test regression suite passes with zero test-file edits; `pnpm generate` output verified byte-identical for the current plugin set; full `pnpm test` (2411 passed) and `pnpm build` green.                                                                                                                                                                                                                   |
| 0.6     | August 2026 | Leg 5 shipped (task 2.17). Split the 891-line `runtime/middleware.ts` into `runtime/src/middleware/{response,session,plugin-gate}.ts` exactly per the epic's list; the new `verifySession()` consolidates three previously-duplicated cookie-cache-then-fallback call sites into one typed result, and `fetchRootPluginPrefix` landed in `plugin-gate.ts` alongside the three lookups the epic named. Task 2.16's existing 115-test regression suite passes completely unchanged, plus 22 new focused unit tests across the three modules; full `pnpm test` (2433 passed), typecheck/lint/build green, Edge middleware bundle size unchanged (86.5 kB).                                                                                                                                                                                                                                                                                                                                                              |
| 0.7     | August 2026 | Leg 6 shipped (task 2.18). Measured real self-fetch counts by path type as an enforced test (0/3/1/1 for normal page/plugin route/root/public API) rather than a one-time note; added a 3-second in-process TTL cache for exactly the two lookups the deliverable named (`fetchDisabledPluginIds`, `fetchRootPluginPrefix`), left entitlement/access-policy lookups uncached per the epic's own guidance; fail-open deliberately extends to the cache (a failure's safe fail-open result is itself cached); added a `resetPluginGateCacheForTests()` hook wired into the regression suite's `beforeEach`. All 115 regression tests pass unchanged, plus 4 measurement and 4 cache-behavior tests; full `pnpm test` (2446 passed).                                                                                                                                                                                                                                                                                    |
| 0.8     | August 2026 | Leg 7 shipped (task 13.6, scoped). Confirm-dialog migration turned out to be already done before the leg started (closed by inspection, no code change); table styling documented in code as staying bespoke (the shared `Table` cell components don't merge an incoming `className`); icon-only and text+icon action-button families consolidated to two shared bases + tone-only overrides via CSS Modules `composes` (zero TSX changes). Manually re-verified live against a disposable test account promoted to owner in the dev DB — deactivate/reactivate cycle, both confirm dialogs, and the consolidated icon buttons all confirmed working with no regression. Also fixed a stale doc-drift reference found along the way: the epic named Task 9.13 as the nav/header migration's blocker; the real blocker was Task 9.15 (corrected in the epic doc). Nav/header migration stays deferred to a follow-up per this workstream's own locked scoping.                                                        |
| 0.9     | August 2026 | Leg 8 shipped (task 3.25, RFC 0057). `bin/plugin-deps.ts` hoists/prunes a plugin's external npm deps into `runtime/package.json` on `sv plugin add`/`remove` and self-heals `.local` plugin deps on every `pnpm dev` boot, backed by a committed `runtime/generated/plugin-deps.json` ledger (the one deliberate carve-out from that directory's blanket `.gitignore` rule); the decision logic lives in pure, unit-tested functions (27 new tests). The `@dnd-kit/*` cleanup deliverable turned out to already be satisfied (entries already absent; initial ledger is `{}`). Verified live against this environment's four real `.local` dev plugins, then deliberately reverted the resulting `runtime/package.json`/lockfile changes before committing (not part of the PR's diff). Full `pnpm test` (2473 passed), typecheck/lint/format/build green. All 8 legs done — workstream complete; `ROADMAP.md`'s Non-prioritised section no longer carries this backlog as unowned.                                  |
