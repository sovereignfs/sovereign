...
| 0.8     | August 2026 | **Reactivated.** Workstream [0019](0019-warden-byo-model-providers.md) — bring-your-own model providers, its own leg 2 (task 22.5) — removed `plugins/warden/manifest.json`'s `disabled: true` once Warden no longer depended solely on the local `apps/harness` engine, verified end to end against a real provider before the flag came off. `plugins/warden/manifest.json` now carries `"disabled": false`; two further workstreams (0019, 0021 — multi-session UI) have since shipped on top of this foundation, extensively featured in `CLAUDE.md`'s own changelog through September 2026. This entry supersedes v0.7's "taking the shipped result out of reach" framing — Warden is live, not deprioritized.                                                                                                                                                                                                                                                                                              |
