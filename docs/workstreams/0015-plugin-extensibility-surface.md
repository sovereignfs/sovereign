...
**Status:** 🔄 In progress — legs 3+5 merged to `main` together at `0.87.0`
(PR #445). Leg 1 (PR #447) is implemented and rebased onto `main` — targets
`0.89.0`, not yet merged; its own PR needed a second rebase after an
unrelated docs-only PR independently claimed `0.88.0` in the interim (see
leg 1's own changelog entry below). Leg 2 is implemented on its own
not-yet-merged branch and will need the same reconciliation exercise when it
rebases; leg 4 is implemented on its own branch stacked on workstream 0017
and has the same reconciliation ahead of it\
**Date:** August 2026\
**Author:** kasunben\
**Goal owner:** kasunben\
