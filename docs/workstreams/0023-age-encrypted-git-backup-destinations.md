...
**Status:** ✅ Complete — all six legs done (tasks 8.37–8.42; leg 4 was the gate leg and passed; legs 5–6 were both verified against real Postgres infrastructure, not just a native `pnpm dev` checkout — see each leg's own changelog entry).\
**Date:** August 2026\
**Author:** kasunben\
**Goal owner:** kasunben\
...
- [x] An operator can restore an age-recipient-encrypted instance backup via
      `sv restore --age-identity <file>`, decrypting with a key that never
      touched the running instance's process or disk. (Leg 6 — verified live
      against real Postgres infrastructure: 4 real schemas and 58 tables
      restored from a genuine recipient-mode archive.)
...
| 0.5     | September 2026 | Leg 6 (task 8.42) shipped — the workstream's final leg. `sv restore` gains `--age-identity <file>`, decrypting a recipient-mode archive (via a new local `decryptWithIdentity()`, `age-encryption` added as `bin/`'s first real npm dependency beyond `citty`/`consola`) into a temp file the existing, unmodified restore logic then reads. A new `parseIdentityFile()` tolerates `age-keygen`'s default comment-line output format, matching the real `age` CLI's own identity-file convention. Verified live against real Postgres infrastructure, reusing leg 5's own generated identity and archive: a fresh `tools` image restored 4 real schemas/58 tables from the recipient-mode archive; the plain (no-flag) path was separately confirmed completely unaffected by restoring an unencrypted archive into a second throwaway database with identical results; a non-matching identity failed cleanly before ever reaching `pg_restore`. Also corrected a stale `docs/self-hosting.md` "Backups" note (predating this workstream) that read as if no backup is ever encrypted automatically. This completes workstream 0023 in full.                                                                                    |
