...
| Key generation                      | Operator: `age-keygen`, entirely outside Sovereign, on the operator's own machine — Sovereign only ever receives the public recipient string. Per-user: client-side in the browser via WASM (`age-encryption.js`), private key shown once for download, never transmitted                                                                                                                                                                                                                                                | Server-generated-then-shown-once for either case — the server would transiently possess the private key in process memory, even if never persisted; a strictly weaker guarantee than "the server is architecturally incapable of decrypting"                                                                                            |
| Sovereign never holds a private key | Architectural invariant, both scopes, no exceptions — decryption of a recipient-mode backup happens only in the operator's own `sv restore` invocation (operator-held key file) or in the user's own browser (client-side WASM)                                                                                                                                                                                                                                                                                          | A restricted-access server-side key store for emergency recovery — rejected as scope creep on this workstream; if wanted later it is an explicit, separate, opt-in escrow feature, not a default                                                                                                                                        |
...
| CSP                                 | Add `'wasm-unsafe-eval'` to `script-src` in `runtime/src/security.ts` — a narrower, WASM-specific token distinct from the general `'unsafe-eval'` that file already deliberately never ships in production                                                                                                                                                                                                                                                                                                               | Server-side decrypt to avoid the CSP change entirely — rejected, see the restore UX row above; this is the one concrete cost of keeping the private key client-side                                                                                                                                                                     |
...
Leg 4 is marked a gate: if client-side WASM decrypt proves unworkable (see
...
- `age-encryption.js` (pure JS/WASM, same author as the `age` spec/CLI) is the
...
- New Account UI (likely `plugins/account/app/data/` alongside the existing
  ...
  "Generate a backup key" — runs `age-encryption.js` in-browser, shows the
...
scope; if `age-encryption.js`'s browser API shape makes this hard to
...
- Client-side decrypt: `age-encryption.js` in the browser, streaming
  ...
  remembering it.
- CSP: add `'wasm-unsafe-eval'` to `script-src` in `runtime/src/security.ts`
  (currently `'self' 'nonce-${nonce}' ${THEME_SCRIPT_CSP_HASH}` in production,
  no `'unsafe-eval'` — this is a narrower, additive token, not a reversal of
  that rule).
...
**Do not proceed if:** client-side WASM decrypt cannot be verified reliable
...
- **The production Docker sv-CLI-spawn gap already blocks workstream 0004
  leg 2 itself** (per 8.16's own progress note) — this workstream's legs 5–6
  inherit that blocker rather than introduce it, but it means the
  operator-scope half of this workstream has a real, currently-open external
  dependency this workstream cannot resolve on its own.
...
- **Losing a personal age identity is unrecoverable by design** — there is
  ...
  overselling it.
- **Client-side WASM decrypt performance on real mobile devices is
  unverified until leg 4** — `age-encryption.js`'s streaming construction
  should handle this, but "should" isn't "verified"; this is exactly why
  leg 4 is a gate rather than an assumption.
...
**If leg 4's client-side WASM decrypt cannot be made reliable and reasonably
