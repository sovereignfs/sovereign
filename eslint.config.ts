...
export default tseslint.config(
  // Ignore generated, build, and dependency output across the monorepo.
  {
    ignores: [
      ...
      'runtime/generated/**',
      // tsup bundle of bin/sv-backup-cli.ts (epic task 8.16, `pnpm build:cli`)
      // — third-party code (citty/consola) is inlined via `noExternal`, so
      // this output fails several rules that only make sense for hand-written
      // source. Built fresh inside the Dockerfile, never committed (.gitignore).
      'runtime/dist-cli/**',
      // PWA service worker, generated by runtime/scripts/build-sw.ts at build.
