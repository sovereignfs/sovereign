...
describe('plugin jobs generation', () => {
  ...
  it('resolves a job to a composed-route-tree import path', () => {
    const generatedDir = join(root, 'runtime', 'generated');
    ...
    expect(decl?.importPath).toMatch(/^\.\..*\(plugins\)\/example\/_jobs\/sync-remote$/);
    expect(decl?.importPath.endsWith('.ts')).toBe(false);
  });
    ...
    const content = renderPluginJobs([
      {
        ...
        type: 'sync.remote',
        maxAttempts: 5,
        importPath: '../app/(platform)/(plugins)/example/_jobs/sync-remote',
      },
    ]);

    expect(content).toContain('AUTO-GENERATED by scripts/generate-registry.ts');
    expect(content).toContain(
      'import handler0 from "../app/(platform)/(plugins)/example/_jobs/sync-remote";',
    );
    expect(content).toContain('pluginId: "com.example.plugin"');
...
describe('plugin events (channel authorizer) generation', () => {
  ...
  it('resolves an event authorizer to a composed-route-tree import path', () => {
    const generatedDir = join(root, 'runtime', 'generated');
    ...
    expect(decl?.importPath).toMatch(/^\.\..*\(plugins\)\/example\/_events\/authorize-list$/);
    expect(decl?.importPath.endsWith('.ts')).toBe(false);
  });
    ...
    const content = renderPluginEvents([
      {
        pluginId: 'com.example.plugin',
        pattern: 'list:*',
        importPath: '../app/(platform)/(plugins)/example/_events/authorize-list',
      },
    ]);

    expect(content).toContain('AUTO-GENERATED by scripts/generate-registry.ts');
    expect(content).toContain(
      'import handler0 from "../app/(platform)/(plugins)/example/_events/authorize-list";',
    );
    expect(content).toContain('pluginId: "com.example.plugin"');
